...
Append one JSON line to `reflections.jsonl` in the agent's own memory directory. Use exact keys `date`, `task`, `outcome`, `surprise`, and `next-time`. Parse every line. Validate the final keys.
