...
Stop ports 5179 and 5180. Verify that the resolved scratch `output/` path is inside the owned worktree. If it is inside, remove the directory. Append one valid JSON line to `reflections.jsonl` in the agent's own memory directory. Use exact keys `date`, `task`, `outcome`, `surprise`, and `next-time`. Validate every JSONL line. Confirm the final Git state.
