...
Use `apply_patch` to append one JSON object to `reflections.jsonl` in the agent's own memory directory. Use keys `date`, `task`, `outcome`, `surprise`, and `next-time`. Parse every non-empty line with `ConvertFrom-Json`. Select the exact task string. Assert that all five keys are present.
