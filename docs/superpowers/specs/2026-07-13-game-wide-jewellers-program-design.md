...
## 2026-07-15 away-plan onboarding closure

The school-week contract promised an away-plan lesson on the first Monday stand-up, but the live PC-to-room callback only restored the crosshair and pause state. Career v1 now carries `tutorials.awayPlanSeen`. Fresh careers start false; legacy v1 saves with no tutorial block migrate to false; present malformed tutorial data still fails closed; night and week folds preserve the flag; Full Reset clears it with the existing whole-career key.

The first Monday PC-to-room transition marks the flag in memory before attempting storage, then presents one 6,500ms neutral polite-status toast. Initial room load, PC entry, later stand-ups, reload after success, and Tuesday through Friday remain silent. The first capture exposed a weak two-line orphan in the 103-character instruction. The final 85-character cue—`Auto-pilot engaged. This is definitely allowed. Set the CRT AWAY PLAN before leaving.`—preserves the promised joke and names both the physical CRT and control in one clean line without changing CSS.

The production-browser contract enters the CRT through the real chair interaction at 900x400, injects simultaneous Mum prompt and subtitle pressure, and pins exact copy, neutral tone, polite live-region attributes, stored seen state, viewport bounds, zero overlap, expiry, same-session suppression, reload suppression, and Tuesday silence. Adversarial review found no surviving issue across the Career or mode-transition fan-out; game-clock expiry intentionally freezes during pause with every existing HUD timer. Full feature-tree verification passes 19 test files / 223 tests, 28 isolated browser scenarios, the full interaction E2E, and standalone and mounted builds. Standalone artifacts are 756,286 raw / 203,408 gzip bytes of JavaScript and 41,737 raw / 10,091 gzip bytes of CSS, inside the unchanged ceilings. Ignored proof is `shots/away-plan-tutorial-1440x900.png` and `shots/away-plan-tutorial-900x400.png`; both were visually reviewed with zero capture-time console warnings or errors.

## 2026-07-15 away-plan command-strip closure

The onboarding cue exposed the next confirmed presentation defect: Mudwick's actual away plan was four translucent debug-like slots labelled `wrk / eat / run / sel`, with no group name, weak OFF contrast, and no hover treatment. The world viewport now carries one opaque hard-pixel `AWAY PLAN` plate with full `WORK / EAT / HOME / SELL` labels. OFF, hover, and ON each have authored contrast; ON also gains a two-pixel state bar so meaning is not colour-only. Drawing and hit-testing consume the same immutable four rectangles, while the existing simulation, click sound, all-OFF defaults, persistence, and away-plan behavior remain unchanged.

The original unit contract failed on both absent exports and the old geometry; the production build exposed terrain at the planned plate anchor. The durable browser guard now pins the opaque plate and OFF fills, all four client-scaled hover/click paths, ON fills and state bars, inclusive/exclusive hit edges, and the untouched side-panel boundary. Adversarial review then reproduced a real sibling collision: a rising `+25 Woodcutting` drop changed 243 pixels inside the strip. XP motion now begins below the plate, and the same browser scenario requires zero changed plate pixels during the transient reward.

Full feature-tree verification passes 19 test files / 226 tests, 29 isolated browser scenarios, the full interaction E2E, and standalone and mounted builds. Standalone artifacts are 756,895 raw / 203,637 gzip bytes of JavaScript and 41,737 raw / 10,091 gzip bytes of CSS, inside the unchanged 204,800-byte and 10,112-byte ceilings; the stylesheet SHA-256 remains `E1E4706C5B04D8A5B4E3E754B25E1F2B03FBB5BDDBC7139FEE6C61090A3933B3`. Ignored production proof is `shots/away-plan-strip-off.png`, `shots/away-plan-strip-hover.png`, `shots/away-plan-strip-mixed.png`, `shots/away-plan-strip-900x400.png`, and `shots/away-plan-strip-xp-clear.png`.
